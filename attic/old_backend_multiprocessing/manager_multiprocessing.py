...
class MultiprocessingManager(Manager):
    ...
    def process_init(self) -> None:
        ...
        self.pool = Pool(processes=self.num_processes, initializer=worker_initialization, **kwargs)
    ...
    def get_resources_status(self):
        ...
        if time_from_last < min_interval:
            resource_available["soft"] = (False, "interval: %.2f < %.1f" % (time_from_last, min_interval))
        else:
        ...
        if not process_limit_ok:
            resource_available["nproc"] = (False, "max %d nproc" % (self.max_num_processing))
            ...
            if num_processing > 0:  # at least one
                if ncpus > 2:
                    ...
                    if max_cpu_load < 100 and estimated_cpu > max_cpu_load:
                        reason = "cpu %d%%, proj %d%% > %d%%" % (max_cpu, estimated_cpu, max_cpu_load)
    ...
    def instance_job(self, job_id):
        ...
        async_result = self.pool.apply_async(parmake_job2, [(job_id, self.context, tmp_filename, False)])
...
class AsyncResultWrap(AsyncResultInterface):
    ...
    def ready(self):
        ...
        if self.count > 100000:
            raise HostFailed(host="localhost", job_id=self.job_id, reason="Timeout", bt="")
