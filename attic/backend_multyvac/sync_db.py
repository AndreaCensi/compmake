...
def synchronize_db_up(context, targets):
    ...
    # XXX: not all jobs
    for job_id in jobs:
        resources = [job2jobargskey, job2userobjectkey, job2cachekey, job2key]
        for r in resources:
            key = r(job_id)
            if key in db:
                keys.append(key)
    ...
    multyvac_job_id = multyvac.submit(copy_files, filename2contents, _vol=[vol.name])
...
def delete_db_volume(db):
    ...
    multyvac_job_id = multyvac.submit(delete_entries, entries, _vol=[vol.name], _name="Reset Compmake DB")
    multyvac_job = multyvac.get(multyvac_job_id)
    multyvac_job.get_result()
