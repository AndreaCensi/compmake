...
def list_jobs_with_state(state, context, cq):
...
def list_ready_jobs(context, cq):
...
def list_uptodate_jobs(context, cq):
...
def list_generated_jobs(context, cq):
...
def list_dynamic_jobs(context, cq):
    """ Returns a list of jobs that are uptodate
        (DONE, and all depednencies DONE)."""
    for job_id in cq.all_jobs():
        job = cq.get_job(job_id)
...
def list_bottom_jobs(context, cq):
...
add_alias('all', lambda context, cq: cq.all_jobs())  
...
add_alias('done',
          lambda context, cq: list_jobs_with_state(Cache.DONE, context=context,
                                                   cq=cq))
add_alias('in_progress',
          lambda context, cq: list_jobs_with_state(Cache.IN_PROGRESS,
                                                   context=context, cq=cq))
add_alias('not_started',
          lambda context, cq: list_jobs_with_state(Cache.NOT_STARTED,
                                                   context=context, cq=cq))
