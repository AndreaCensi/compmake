...
def up_to_date(job_id: CMJobID, db: Storage) -> Tuple[bool, str]:
    """
    ...
    *) we are in the up_to_date_cache
    ...
    """
    cq = CacheQueryDB(db)
    res, reason, _ = await cq.up_to_date(job_id)
    return res, reason


#
# def direct_uptodate_deps(job_id: CMJobID, db: Storage) -> Set[CMJobID]:
...
async def direct_uptodate_deps_inverse(job_id: CMJobID, db: Storage) -> Set[CMJobID]:
    ...
    dep_inv = await direct_parents(job_id, db)
    ...
    if cache.state == Cache.DONE:
        dep_inv.update(await jobs_defined(job_id, db))
    return dep_inv


def direct_uptodate_deps_inverse_closure(job_id: CMJobID, db: Storage) -> Set[CMJobID]:
    """
    Closure of direct_uptodate_deps_inverse:
    all jobs that depend on this.
    """
    # all parents
    dep_inv = await parents(job_id, db)
    # plus their definition closure

    closure = await definition_closure(dep_inv, db)
