...
async def dump(sti, non_empty_job_list, context, directory="."):
    ...
    for job_id in non_empty_job_list:
        ...
        if await is_job_userobject_available(job_id, db=db):
...
async def dump_stdout(sti, non_empty_job_list, context, resolve=False):
    ...
    for job_id in non_empty_job_list:
        if await is_job_userobject_available(job_id, db=db):
            if resolve:
                user_object = await get_job_userobject_resolved(job_id, db)
            else:
                user_object = await get_job_userobject(job_id, db=db)
